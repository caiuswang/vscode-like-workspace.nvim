...
function Workspace:load()
    ...
    if not ok then
        log.info("Workspace file not found: " .. self.file_path)
        return
    end
    ...
    -- Use vim.print for table inspection
    -- Debug path resolution
    -- local debug_path = Path:new(self.file_path):parent().fsname
    -- vim.notify("Resolved workspace root: " .. debug_path, vim.log.levels.DEBUG)
